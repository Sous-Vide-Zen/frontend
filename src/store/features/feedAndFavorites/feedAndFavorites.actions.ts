...
import { FetchListData } from './feedAndFavorites.types'
...
export const feedAndFavorites = mainApi.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    getRecipes: builder.query<FetchListData, ListParams>({
    ...
    getFavorites: builder.query<FetchListData, ListParams>({
      keepUnusedDataFor: 1, // кэш сохраняется 1 сек
      query: ({ pathname, params }) => {
        const fixedParams = params as Record<string, string>
        return {
    ...
    addToFavorites: builder.mutation<void, string>({
      ...
      async onQueryStarted(slug, { dispatch, queryFulfilled }) {
        ...
        getListParamsVariants.forEach(({ pathname, params }) => {
          dispatch(
            feedAndFavorites.util.updateQueryData(
              'getRecipes',
              {
                pathname,
                params,
